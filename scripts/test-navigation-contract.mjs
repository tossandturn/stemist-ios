...
assert.match(webModule, /kind:\s*['"]camera-direct['"][\s\S]{0,120}?requestId/, 'camera controls must request a direct native capture on iOS versions that cannot customize file panels')
assert.match(webModule, /event\.preventDefault\(\)[\s\S]{0,120}?event\.stopImmediatePropagation\(\)/, 'the direct camera bridge must suppress the WebKit file picker only after native handoff succeeds')
...
assert.match(webModule, /message\.frameInfo\.isMainFrame/, 'native camera messages must be restricted to the trusted main frame')
assert.match(webModule, /directCameraRequestID/, 'native camera capture must retain the exact originating input request')
...
assert.match(webModule, /maxCameraDimension:\s*CGFloat\s*=\s*2048/, 'camera images must be resized before crossing the JavaScript bridge')
assert.match(webModule, /maxCameraAttachmentBytes\s*=\s*4\s*\*\s*1024\s*\*\s*1024/, 'camera images must have a hard bridge payload limit')
assert.match(webModule, /new DataTransfer\(\)/, 'native camera data must be restored to the originating web file input')
assert.match(webModule, /dispatchEvent\(new Event\(['"]change['"]/, 'camera attachment must trigger the existing web upload pipeline')
